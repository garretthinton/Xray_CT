...
void CTRunAction::BeginOfRunAction(const G4Run*)
{ 
...
void CTRunAction::EndOfRunAction(const G4Run* run)
{
  ...
  const CTDetectorConstruction* detectorConstruction
   = static_cast<const CTDetectorConstruction*>
  ...
  const CTPrimaryGeneratorAction* generatorAction
   = static_cast<const CTPrimaryGeneratorAction*>
...
void CTRunAction::AddEdep(G4double edep)
{
  fEdep  += edep;
  fEdep2 += edep*edep;
}
